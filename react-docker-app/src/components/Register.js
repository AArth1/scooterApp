...
//import userAuthData from './UserAuthentication.json';
...
function Register() {
  ...
  const userAuthData = fetch('./UserAuthentication.json');

  const handleRegister = () => {
    // check if username already exists in UserAuthentication.json
    const userExists = userAuthData.UserAuthentication.some(user => user.Username === username);
    if (userExists) { // if username exists, make user input a different username
      setMessage('This username already exists. Please choose a different username.');
    } else { // if username does not exist, add new user to UserAuthentication.json
      makeNewUser();
